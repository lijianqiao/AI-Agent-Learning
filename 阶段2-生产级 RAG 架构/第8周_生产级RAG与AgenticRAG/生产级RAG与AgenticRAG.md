...
## 第五部分学习：长上下文 vs RAG 决策矩阵

### 1M Token 窗口的到来，RAG 还有用吗？

2025 年，Gemini 3 Pro、Claude 4.5 Sonnet、GPT-4o 相继支持 100K-2M Token 上下文窗口。很多人开始质疑：**既然可以把整个知识库塞进上下文，RAG 还有存在的必要吗？**

答案是：**有，而且非常必要**——但适用场景不同了。

### 三种架构的核心对比

| 维度 | 纯长上下文（Long Context） | 纯 RAG | 混合（Contextual Retrieval） |
| --- | --- | --- | --- |
| **工作原理** | 把所有文档塞进 Prompt | 检索最相关的 Top-K 片段 | RAG + 在每个 Chunk 中注入文档摘要上下文 |
| **召回精度** | 100%（没有召回问题） | 取决于向量相似度，可能漏召回 | 显著优于传统 RAG（Anthropic 数据：召回失败率降低 49%） |
| **推理成本** | 极高（1M tokens ≈ $5-15 每次查询） | 低（通常 1K-5K tokens） | 中等（索引构建贵，查询便宜） |
| **延迟** | 较高（长上下文推理慢） | 低（快速检索 + 短上下文） | 中等 |
| **知识更新** | 重新注入，灵活 | 需重建索引，成本高 | 需重建索引 |
| **适合规模** | < 500 个文档（书籍级） | 百万级文档库 | 数千至百万文档 |
| **精准溯源** | 困难（模型不会自动标注来源） | 天然带 Reference | 天然带 Reference |

### 生活类比：工具书 vs 图书馆 vs 带目录的工具书

**纯长上下文** = 把整本《百科全书》摆在你桌上，翻哪页都行，但带着这本书走路很累（成本高）。

**纯 RAG** = 去图书馆检索系统找书，快而准，但检索关键词不对就找不到（召回失败）。

**Contextual Retrieval** = 每本书的每一章开头都印着"本章是全书第X章，讲的是XX主题，整书讲的是XX"——这样即使你只拿到一章，也能理解它的上下文含义，检索精度大幅提升。

### Anthropic Contextual Retrieval 核心原理

传统 RAG 的召回失败，很多时候是因为 Chunk 失去了上下文。

```
# 传统 Chunk（失去上下文）
"公司的营收同比增长了 32%。"
# 这个 Chunk 根本不知道"哪家公司"、"哪个季度"，向量检索容易失效

# Contextual Retrieval 处理后的 Chunk
"【本文档背景：苹果公司2024年Q3财报，发布于2024年10月。本章节讲述营收情况。】
 公司的营收同比增长了 32%。"
# 加入上下文后，检索精度大幅提升
```
...
```python
def add_context_to_chunk(document: str, chunk: str, llm_client) -> str:
    """使用 LLM 为每个 Chunk 注入文档级上下文"""
    prompt = f"""
    <document>
    {document}
    </document>
    ...
    请用1-2句话描述这个片段在整个文档中的位置和上下文，
    以便在脱离文档的情况下也能理解这个片段的含义。
    只输出上下文描述，不要重复片段内容。
    """
    context = llm_client.ask(prompt)
    return f"【上下文：{context}】\n\n{chunk}"
```

> ⚠️ 注意：Contextual Retrieval 在索引构建阶段需要为每个 Chunk 调用一次 LLM，成本较高，适合文档更新不频繁的场景。Anthropic 建议配合 Prompt Caching 降低成本。

### 三轴决策矩阵

```
                    知识库规模
                    小（<1K文档）    中（1K-100K）    大（>100K）
                   ┌────────────────┬───────────────┬───────────────┐
更新频率  高（每天） │  长上下文 ✅   │  RAG ✅       │  RAG ✅       │
          中（每周） │  长上下文 ✅   │  混合 ✅      │  RAG ✅       │
          低（每月） │  长上下文 ✅   │  混合 ✅      │  混合 ✅      │
                   └────────────────┴───────────────┴───────────────┘
```
...
- **需要精准溯源** → 优先 RAG / 混合（长上下文无法自动给出 Reference）
- **成本敏感** → 优先 RAG（每次查询 Token 少得多）
- **任务需要"全局理解"** → 优先长上下文（如"总结整本书的主旨"）

### 架构师的实用决策公式

```
if 文档总量 < 200页 AND 不需要溯源 AND 成本不敏感:
    → 纯长上下文（简单暴力，效果好）
elif 文档库 > 10万条 OR 需要实时更新:
    → 纯 RAG + Hybrid Search + Reranker
else:
    → Contextual Retrieval（最佳平衡点）
```
