...
| Agent Skills 范式 | 能否说清 Skills vs Tools 的 3 个核心区别？ | ☐ |
| SKILL.md 规范 | 能否写出一个完整规范的 SKILL.md 文件？ | ☐ |

---

## 第五部分学习：Agent Skills 范式

### Skills 是什么？为什么不只是 Tools？

**Tools** 是 Agent 工具箱里的**原子操作**：

- `search_web(query)` → 搜索网页
- `read_file(path)` → 读取文件
- `send_email(to, subject, body)` → 发送邮件

每个 Tool 只做一件事，无状态，无流程。

**Skills** 是**封装完整工作流的可复用能力单元**：

- "市场调研 Skill" = 搜索 → 筛选 → 分析 → 汇总报告（多个 Tools 的编排）
- "代码审查 Skill" = 读取代码 → 分析问题 → 给出建议 → 写入报告（有状态的工作流）
- "竞品监控 Skill" = 定时触发 → 爬取 → 对比分析 → 推送通知（触发条件 + 多步骤）

### 生活类比：锤子 vs 装修方案

**Tools** 就像各种单件工具——锤子、电钻、油漆刷。每件工具只做一件事。

**Skills** 就像"装修卧室方案"——包含了：先刷墙（用什么漆、刷几遍），再铺地板（流程），再装灯（注意事项），最后验收（验收标准）。是一套完整的、可复用的、有说明书的**工作流程包**。
...
| **状态** | 无状态 | 可有中间状态 |
| **可复用层级** | 低层可复用 | 高层业务可复用 |
| **描述方式** | 函数签名 + docstring | `SKILL.md` 规范文档 |
| **触发方式** | Agent 直接调用 | Agent 判断场景后调用整个 Skill |
| **典型代表** | Function Calling | Cursor SKILL.md、OpenAI 自定义 GPT Actions |

### SKILL.md 规范结构

`SKILL.md` 是一种正在逐渐标准化的 Skill 描述格式——本质上是给 AI Agent 看的"工作说明书"：

```markdown
# 市场调研 Skill

## 适用场景（触发时机）
当用户需要了解某个产品、竞品、行业的市场情况时使用。

## 前置条件
- 需要明确的调研对象（公司名 / 产品名 / 行业名）
- 需要调研目的（投资分析 / 竞品对比 / 进入决策）

## 执行步骤

### 步骤一：信息收集
- 使用 `search_web` 工具搜索最近6个月的新闻和报告
- 关键词："{调研对象} 市场份额"、"{调研对象} 营收"、"{调研对象} 用户评价"
- 至少收集 5 个独立来源

### 步骤二：信息筛选
- 过滤掉超过1年的旧信息
- 标注每条信息的可信度（官方发布 > 媒体报道 > 论坛讨论）

### 步骤三：分析汇总
- 结构化输出：公司概况、核心产品、市场规模、竞争格局、近期动态

### 步骤四：报告生成
- 使用 `write_file` 工具将报告保存为 `report_{调研对象}_{日期}.md`
...
从 Agent 的视角看，Skill 就是一个"高层能力标签"：

```python
# 传统 Tool-Calling Agent：需要 Agent 自己规划每个 Tool 的调用顺序
# 容易在多步骤执行中走偏

# Skill-Enabled Agent：Agent 识别场景，整体调用 Skill
class SkillEnabledAgent:
    def __init__(self):
        self.skills = self._load_skills("./skills/")  # 从 SKILL.md 加载

    def run(self, user_request: str) -> str:
        # 1. 识别最匹配的 Skill
        matched_skill = self._match_skill(user_request)

        if matched_skill:
            # 2. 将 SKILL.md 的工作流注入 System Prompt
            system_prompt = f"""
            你正在执行"{matched_skill.name}" Skill。
            请严格按照以下步骤执行，不要跳过任何步骤：

            {matched_skill.steps}

            输出格式要求：
            {matched_skill.output_format}
            """
            return self.llm.chat(system_prompt, user_request)
        else:
            # 3. 无匹配 Skill，退化为普通 Tool-Calling
            return self.tool_calling_agent.run(user_request)
...
1. **可测试性**：Skill 有明确的输入/输出规范，可以独立测试每个步骤
2. **可复用性**：同一个"市场调研 Skill"可以被产品 Agent、投资 Agent、竞品 Agent 复用
3. **可维护性**：修改工作流只需更新 `SKILL.md`，不需要改代码
4. **可解释性**：每次调用 Skill 都有明确的步骤日志，便于追踪问题
5. **权限控制**：可以为不同用户/场景开放不同的 Skill 集合
